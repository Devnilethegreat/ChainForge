...
import pytest
from chainforge import ChainForgeCore, ChainForge


class TestCore:
    def setup_method(self):
        self.core = ChainForgeCore(threshold=0.75)
    ...
    def test_threshold_respected(self):
        core = ChainForgeCore(threshold=0.99)
        result = self.core.process({"value": 500_000.0, "velocity": 100.0, "count": 20})
        assert isinstance(result["flagged"], bool)


class TestChainForge:
    def test_run_succeeds(self):
        app = ChainForge(verbose=False)
